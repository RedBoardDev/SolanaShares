...
export class GenericCacheServiceImpl implements GenericCacheService {
  ...
  constructor(defaultTtlMs: number = 30 * 60 * 1000) {
    ...
    logger.debug('[GENERIC_CACHE] Service initialized', { defaultTtlMs });
  }

  /**
   * Get singleton instance with default TTL.
   */
  static getInstance(defaultTtlMs?: number): GenericCacheServiceImpl {
    if (!GenericCacheServiceImpl.instance) {
      GenericCacheServiceImpl.instance = new GenericCacheServiceImpl(defaultTtlMs);
    }
    return GenericCacheServiceImpl.instance;
  }
  ...
  async get<T>(key: string): Promise<T | null> {
    ...
    if (!entry) {
      this.missCount++;
      logger.debug('[GENERIC_CACHE] Cache miss', { key });
      return null;
    }
    ...
    if (this.isExpired(entry)) {
      ...
      this.missCount++;
      logger.debug('[GENERIC_CACHE] Cache expired', { key, age: Date.now() - entry.timestamp });
      return null;
    }

    this.hitCount++;
    logger.debug('[GENERIC_CACHE] Cache hit', { key });
  ...
  set<T>(key: string, value: T, ttlMs?: number): void {
    ...
    logger.debug('[GENERIC_CACHE] Cache set', {
      key,
      ttl: effectiveTtl,
  ...
  delete(key: string): void {
    ...
    if (existed) {
      logger.debug('[GENERIC_CACHE] Cache delete', { key });
    }
  }
  ...
  clear(): void {
    ...
    logger.info('[GENERIC_CACHE] Cache cleared', { previousSize });
  }
  ...
  cleanup(): void {
    ...
    if (cleanedCount > 0) {
      logger.debug('[GENERIC_CACHE] Cleanup completed', {
        before,
